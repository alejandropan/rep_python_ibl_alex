...
Edited : 2018-01-03 @alejandro Princeton IBL
   - Edits solves some issues with 2018 pbypod files.
...
def obj_median(behav):
    ...
    for day in days:
        trialcounts.append(behav.rt[(behav.days == day)].median())
...
def plot_performance_rt(behav, ax, xlims):
    ...
    # RTs on right y-axis
    rtimes_l = obj_median(behav) # @alejadro  - It was behav.groupby(['date'])['rt'].median().reset_index() but this doesnt work in the python object Stim_ontimes.  Requires fuction get_obj
    dts = pd.DataFrame({'date':behav.date.unique()})
    rtimes  = pd.concat([dts,pd.DataFrame({'rt':rtimes_l})], axis=1)
    righty = ax.twinx() # @alejadro changed trialcounts to rtimes -  it was confusing
    sns.lineplot(x="date", y="rt", marker='o', color="firebrick", data=rtimes, ax=righty)
